...
export default function ProvidedByMspPanel({ tenantId }: { tenantId: string }) {
    const { data } = useQuery({
        ...
        queryFn: () => AssignmentService.getProvidedResources(tenantId),
    });
            ...
            <div className="divide-y divide-white/70 dark:divide-white/10 text-sm">
                {data.publicIps.map(ip => (
                    <Row key={ip.id} icon={<Globe size={14} />}
                        label={<Link to="/datacenter" className="font-mono hover:text-primary-600">{ip.ipAddress}</Link>}
                        detail={`${ip.usage ?? 'Public IP'} · ${ip.rangeCidr}`} />
                ))}
                {data.vlans.map(v => (
